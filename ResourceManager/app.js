        ...
        class UIManager {
            ...
            renderProjects(projects) {
                ...
                tbody.innerHTML = projects.map(proj => {
                    const recommendationCount = this.getRecommendationCount(proj);
                        ...
                        <td>
                            <div class="action-buttons">
                                ...
                                <div class="action-btn-with-badge">
                                    <button class="icon-btn" onclick="app.editProject('${proj.id}')" title="Edit & Assign">
                                        <i class="fas fa-edit"></i>
                                    </button>
                                    ${recommendationCount > 0 ? `<span class="notification-badge">${recommendationCount}</span>` : ''}
                                </div>
                            </div>
                        </td>
                    </tr>
            ...
            getRecommendationCount(project) {
                // Mock function - in real app, this would calculate based on project skills and available employees
                const counts = {
                    'PROJ001': 3,
                    'PROJ002': 2,
                    'PROJ003': 4,
        ...
        class ResourceManagerApp {
          ...
          async editProject(id) {
                try {
                    ...
                    // Get recommended employees based on project skills
                    const recommendations = this.getEmployeeRecommendations(project, employees);
                    
                    // Get current team members (mock for now)
                    const currentTeam = this.getCurrentTeamMembers(project, employees);

                    this.showEditProjectModal(project, recommendations, currentTeam);
                } catch (error) {
                    this.uiManager.hideLoading();
            ...
            getEmployeeRecommendations(project, employees) {
                // Filter employees who have matching skills
                ...
                // Return at least 3 employees, even if they don't match perfectly
                if (filtered.length >= 3) {
                    return filtered.slice(0, 5);
                } else {
                    // Add more employees to reach at least 3
            ...
            getCurrentTeamMembers(project, employees) {
                // Mock function - in real app, this would fetch actual team members
                return employees
                    .filter(emp => emp.projects.includes(project.name))
                    .slice(0, 3);
            }

            showEditProjectModal(project, recommendations, currentTeam) {
                ...
                document.getElementById('modalProjectStatus').innerHTML = `<span class="project-status ${project.status}">${this.uiManager.capitalize(project.status)}</span>`;
                ...
                // Store current team member IDs for checking
                const currentTeamIds = currentTeam.map(emp => emp.id);

                // Populate recommendations
                document.getElementById('recommendationCount').textContent = `${recommendations.length} Matches`;
                const recommendationsList = document.getElementById('recommendationsList');
                
                if (recommendations.length === 0) {
                    recommendationsList.innerHTML = '<p style="text-align: center; color: var(--text-secondary); padding: 20px;">No matching employees found</p>';
                } else {
                    recommendationsList.innerHTML = recommendations.map((emp, index) => {
                        const isCurrentMember = currentTeamIds.includes(emp.id);
                        return `
                        <div class="recommendation-card-no-image" data-emp-id="${emp.id}">
                            <input type="checkbox" class="employee-checkbox" data-emp-id="${emp.id}" ${isCurrentMember ? 'checked' : ''}>
                            <span class="employee-number">${index + 1}.</span>
                            <img src="${emp.avatar}" alt="${emp.name}" class="employee-avatar-circle">
                            <div class="recommendation-details">
                                <h4>${emp.name}</h4>
                                <p>${emp.role}</p>
                                <div class="recommendation-skills">
                                    ${emp.matchingSkills.map(skill => `<span class="skill-tag">${skill}</span>`).join('')}
                                </div>
                            </div>
                            <span class="match-score">${emp.matchPercentage}% Match</span>
                        </div>
                    `;
                    }).join('');
                }
                ...
                // Hide current team section
                const currentTeamSection = document.querySelector('.current-team-section');
                ...
                document.getElementById('saveProjectTeam').onclick = () => {
                    this.saveSelectedEmployees(project.id);
                    closeModal();
                };
            ...
            async saveSelectedEmployees(projectId) {
                try {
                    const checkboxes = document.querySelectorAll('.employee-checkbox:checked');
                    const selectedEmployees = Array.from(checkboxes).map(cb => cb.dataset.empId);
                    
                    if (selectedEmployees.length === 0) {
                        this.uiManager.showError('Please select at least one employee');
                        return;
                    }
                    ...
                    this.uiManager.hideLoading();
                    this.uiManager.showSuccess(`${selectedEmployees.length} employee(s) assigned to project successfully!`);
                    this.loadProjects();
                } catch (error) {
                    this.uiManager.hideLoading();
                    console.error('Error saving project team:', error);
                    this.uiManager.showError('Failed to save project team');
                }
            }

            async addEmployeeToTeam(empId, projectId) {
                try {
                    this.uiManager.showLoading();
                    ...
                    this.uiManager.hideLoading();
                    this.uiManager.showSuccess('Employee added to team successfully!');
                    ...
                    this.uiManager.showError('Failed to add employee to team');
                }
            }

            async removeEmployeeFromTeam(empId, projectId) {
                try {
                    ...
                    this.uiManager.hideLoading();
                    this.uiManager.showSuccess('Employee removed from team');
                    ...
                    this.uiManager.showError('Failed to remove employee from team');
                }
            }

            async saveProjectTeam(projectId) {
                try {
                    this.uiManager.showLoading();
                    ...
                    this.uiManager.hideLoading();
                    this.uiManager.showSuccess('Project team updated successfully!');
